...
public class PriceDao extends JdbcDao {
    ...
    public void addTradePostData(Price price) {
        update("INSERT INTO price(item_id, created_at, buy_price, buy_quantity, sell_price, sell_quantity) VALUES(?, ?, ?, ?, ?, ?) ON CONFLICT (item_id, created_at) DO UPDATE SET buy_price=?, buy_quantity=?, sell_price=?, sell_quantity=?", price.getId(), Timestamp.valueOf(price.getTime()), price.getBuyPrice(), price.getBuyQuantity(), price.getSellPrice(), price.getSellQuantity(), price.getBuyPrice(), price.getBuyQuantity(), price.getSellPrice(), price.getSellQuantity());
    }
